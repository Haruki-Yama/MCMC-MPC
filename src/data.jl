...
function get_KCS_basic_params(ρ=1025.0)
    L_pp = 3.0464  # 船長Lpp[m]
    B = 0.4265  # 船幅[m]
    d = 0.1430  # 喫水[m]
    nabla = 0.1209  # 排水量[m^3]
    x_G = -0.0451  # 重心位置[m]
    # C_b = 0.810  # 方形係数[-]
    D_p = 0.1046  # プロペラ直径[m]
    H_R = 0.1311  # 舵高さ[m]
    A_R = 0.007944  # 舵断面積[m^2]
    t_P = 0.207  # 推力減少率
    w_P0 = 0.355  # 有効伴流率
    m_x_dash = 0.0061  # 付加質量x(無次元)
    m_y_dash = 0.1521  # 付加質量y(無次元)
    J_z_dash = 0.0089  # 付加質量Izz(無次元)
    t_R = 0.258  # 操縦抵抗減少率
    x_R_dash = -0.500  # 舵の相対位置
    a_H = 0.361  # 舵力増加係数
    x_H_dash = -0.436  # 舵力増分作用位置
    γ_R_minus = 0.338  # 整流係数
    γ_R_plus = 0.492  # 整流係数
    l_r_dash = -0.755  # 船長に対する舵位置
    x_P_dash = -0.480  # 船長に対するプロペラ位置
    ϵ = 0.956  # プロペラ・舵位置伴流係数比
    κ = 0.633  # 修正係数
    f_α = 3.005  # 直圧力勾配係数
...
function get_KCS_maneuvering_params()
    k_0 = 0.5228
    k_1 = -0.4390
    k_2 = -0.0609
    R_0_dash = 0.0167
...
function get_example_KCS_ship_wind_force_moment_params()
    L_pp = 3.0464  # 船長Lpp[m]
    B = 0.4265  # 船幅[m]
    d = 0.1430  # 喫水[m]
    D = 0.238 # 深さ[m]
